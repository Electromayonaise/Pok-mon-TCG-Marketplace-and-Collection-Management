...
### AD-13 — `legalIdentity` handling satisfies Ley 1581 as ordinary personal data, owned by `identity`

- **Binds:** `identity` module (`BusinessApplication.legalIdentity` and its supporting documents)
- **Prevents:** business-verification data being collected/stored with no lawful basis, no access scoping, or no rights-request path — and, separately, over-engineering a heightened "sensitive data" handling regime the law doesn't actually require here
- **Rule:** Colombian Ley 1581 (habeas data) binds the platform as data controller ("Responsable del Tratamiento") for any personal data it collects, regardless of company size — this is not size-gated. `legalIdentity` (cédula/RUT/business-registration identifiers and their uploaded documents) is **ordinary personal data**, not Art. 5 "sensitive data" (which is limited to racial/ethnic origin, political orientation, religious/philosophical convictions, union/social/human-rights-org membership, health, sex life, and biometric data) — sensitive-data treatment would only be triggered if verification did automated biometric matching against the ID photo, which CAP-15's manual admin-review design does not do. Three concrete obligations follow: (1) `BusinessApplication` owns two fields directly on its own aggregate — `legalIdentityConsentGivenAt: DateTime` and `legalIdentityConsentVersion: string` (referencing the Privacy Policy version in effect at submission) — set before `legalIdentity` is persisted, never a separate `ConsentRecord` table, consistent with AD-8's table-ownership rule; (2) a published Privacy Policy / Aviso de Privacidad (static content, not a new module) states purpose, retention, and how to exercise data-subject rights (access/correction/update/deletion/consent-revocation), routed to a designated contact who is documented but need not be a dedicated feature — a contact-form/email channel is proportionate at this team size; (3) `legalIdentity` documents in Supabase Storage are access-scoped to the admin-review path only, per AD-8's table/data-ownership boundary extended to file storage — no other module reads them directly. RNBD (national database registry) registration is not required: it is gated on total assets over 100,000 UVT (~USD $1.1M), far above a self-funded 2-person team's scale, and is revisited only if that changes. **Scope note:** this AD governs `legalIdentity` specifically, not Ley 1581 compliance platform-wide — buyer-account, review, and messaging personal-data consent are a separate, Deferred question, not silently assumed covered by this AD just because SPEC.md's constraint line names Ley 1581 generally.
- **Context & Problem:** SPEC.md flagged `legalIdentity` as regulated personal data needing "lawful-basis/consent and secure-handling practices... deferred to architecture" but never named the mechanism — Repair Plan Step 3 closes that gap. A secondary source consulted during research overstated the risk, implying identity documents trigger Ley 1581's heightened sensitive-data regime; the statutory Art. 5 list does not support that absent biometric matching, so the correct bar is the standard-consent regime, not the heightened one. A follow-up adversarial-divergence pass (`reviews/review-adversarial-divergence-2.md`, Finding 7) found the first draft of this AD named the consent obligation narratively without pinning a field/table shape, the same class of gap the original review's Finding 1 found in event payloads — two builders would have modeled consent-storage differently (a field on `BusinessApplication` vs. a general-purpose `ConsentRecord` table). Fixed by pinning the exact fields below.
- **Decision Taken:** treat `legalIdentity` as ordinary personal data under standard consent rules; capture timestamped, versioned consent as two named fields on `BusinessApplication` itself (never a separate table); publish a Privacy Policy with a designated rights-request contact; scope stored identity documents to admin-review access only; skip RNBD registration until the asset threshold is approached; scope this AD to `legalIdentity` only, not Ley 1581 compliance platform-wide.
- **Status:** Accepted
- **Consequences & Trade-offs:** meaningfully lighter compliance lift than a sensitive-data regime would require (no special heightened-consent UI, no RNBD filing) — but the platform is still legally exposed if verification is later extended to automated biometric matching, which would flip `legalIdentity` into Art. 5 sensitive-data territory and require re-opening this AD. The Privacy Policy and rights-request contact are product/ops deliverables this AD names but does not itself produce — tracked as a Deferred content item, not an unresolved architecture question. A genuine Ley 1581 deletion (supresión) request against an approved business's `legalIdentity` is not resolved here — see Deferred; it sits in tension with AD-12's hide-never-delete convention elsewhere in the spine and needs its own decision once real request volume justifies the design cost.
- **Rejected Alternatives:** (a) treating `legalIdentity` as sensitive data and building a heightened opt-in/consent-versioning flow — unsupported by the statutory Art. 5 list for the platform's actual (non-biometric) verification design; would be premature engineering. (b) registering with the RNBD preemptively — not required below the UVT asset threshold; unnecessary overhead for a pre-revenue team. (c) a general-purpose `ConsentRecord(userId, purpose, version, givenAt)` table anticipating future reuse beyond `legalIdentity` — reasonable in principle given Ley 1581's actual platform-wide scope, but out of scope for this AD (see Scope note) and premature before a second consent-needing flow is actually designed.
...
| Vercel | free/Hobby tier (hosting, CI/CD, PR preview environments) — confirmed non-compliant with Hobby's commercial-use ToS (commission processing counts); team accepted the risk and stayed on Hobby rather than upgrade, see Deferred |
...
| CAP-5, CAP-15 (business verification, admin review) | `identity` | AD-1, AD-13 (Ley 1581 handling of `legalIdentity`) |
...
- **Resolved (Repair Plan Step 2, 2026-09-01):** Vercel's Fair Use Guidelines and ToS §4 define commercial usage as any deployment used "for the purpose of financial gain of anyone involved," including any payment-processing method — this is confirmed to cover the platform, since the single-deployable modular monolith has commission processing (AD-3) built in from day one, not a later add. **Vercel Hobby tier is confirmed non-compliant.** Team decision: accept the risk and stay on Hobby rather than upgrade to Pro (~$20/mo) or switch providers — a deliberate bet on low enforcement likelihood pre-revenue, not a compliance fix. Vercel's enforcement is fully discretionary (deployment can be disabled "with or without notice... for any reason or no reason" per ToS §4); this is the accepted risk, and it should be revisited once real revenue/traffic makes the platform more visible.
- Privacy Policy / Aviso de Privacidad page content and the designated Ley-1581 rights-request contact (AD-13) — product/ops content deliverable, not an architecture component; needs to exist before CAP-15's business-application flow collects `legalIdentity` data in production.
- Ley 1581 deletion (supresión) request handling for an approved business's `legalIdentity`, and its interaction with AD-12's hide-never-delete convention (`review-adversarial-divergence-2.md` Finding 8) — likely needs a lawful-retention-exception carve-out (data needed to substantiate a business's past verified transactions) rather than either pure soft-delete or pure hard-delete; not decided now, a low-volume edge case resolved at implementation time once real request volume justifies the design cost.
